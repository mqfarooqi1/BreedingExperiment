...
test_that("it behaves as a RangedSummarizedExperiment", {
  be <- simulateBreeding(n_ind = 12, n_marker = 40)
...
test_that("markers can be selected by genomic region", {
  be <- simulateBreeding(n_ind = 10, n_marker = 50)
...
test_that("accessors round-trip", {
  be <- simulateBreeding(n_ind = 10, n_marker = 20)
  g <- genotypes(be)
  genotypes(be) <- g
...
test_that("allele frequency summaries are in range", {
  be <- simulateBreeding(n_ind = 25, n_marker = 60, missing = 0.05)
...
test_that("quality control filters and imputes", {
  be <- simulateBreeding(n_ind = 30, n_marker = 120, missing = 0.08)
...
test_that("show() prints a summary", {
  be <- simulateBreeding(n_ind = 8, n_marker = 15)
  expect_output(show(be), "BreedingExperiment")
  expect_output(show(be), "pedigree")
})

test_that("the simulator honours its arguments", {
  be <- simulateBreeding(n_ind = 30, n_marker = 80, genotyped = 12)
  expect_equal(ncol(be), 12L)
  expect_equal(nrow(pedigree(be)), 30L)
  set.seed(9); a <- simulateBreeding(n_ind = 10, n_marker = 20)
  set.seed(9); b <- simulateBreeding(n_ind = 10, n_marker = 20)
